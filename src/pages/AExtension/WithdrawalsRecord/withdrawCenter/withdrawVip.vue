    ...
    <div class="item">
      <div class="item-title">
        <div>
          ...
          <span style="font-size:0.3rem">{{vipNumber}}</span>
          人，已提取奖励金
          <span style="font-size:0.3rem">{{cashVipTimes }}</span>次
        </div>
        <p style="margin:0.16rem 0 0.26rem 0">我的奖励金&nbsp;(元)</p>
        <p style="font-size:32px;font-weight:800;">{{money.toFixed(2)}}</p>
      </div>
      <div class="item-main">
        <div class="info-item">
...
line-height:0.48rem;"
      >
        <div class="p">
          <p>1、推广vip，享全额vip奖励金；</p>
          <!-- <p v-if="level==2">1、享有150个VIP招募权限，完成招募可获得奖励金2985元。每招募15人可提取奖励金298.5元，可提取10次；</p>
          <p v-if="level==3">1、享有1800个VIP招募权限，完成招募可获得奖励金35820元。每招募15人可提取奖励金298.5元，可提取120次；</p>-->
      ...
      this.$http
        ...
        .then(res => {
          if (res && res.data && res.data.resultMap) {
            ...
            _this.vipNumber = resultMap.vipNumber;
            _this.money = resultMap.pMoney;
            // _this.level = resultMap.level;
            _this.cashVipTimes = resultMap.cashVipTimes;
...
.item > .item-title {
  ...
  background-image: url("../../../../assets/images/withdraw-bg.png");
