<template>
  <div style="background:#fafafa;width:100%;background:rgba(255,252,237,1);position: relative">
    <div class="top-banner">
      <img src="../../../assets/images/memberUp/memberUp-banner.png" alt />
    </div>
    <div class="promotionback">
      ...
      <div class="prospeed">
        <div class="each-prospeed" v-if="level==1" @click="checktype(1)">
          <img
            v-if="!oneactive"
          ...
          <img src="../../../assets/images/payPromotioncenter-img-gou.png" class="img-gou" alt />
        </div>
      ...
      <div class="projieshao" v-if="oneactive">
        <div class="juese">您已选择：998高级合伙人</div>
        <div class="juese">
          申请会费：
          <span style="color:#E94829;margin-left:-0.08rem">￥998.00</span>
        </div>
        <div class="juese">
          您的任务：招募<span style="color:#E94829">{{ruleInfo2.downMax}}</span>名合伙人
        </div>
      </div>
      <div class="projieshao" v-if="twoactive">
        <div class="juese">您已选择：9980高级合伙人</div>
        <div class="juese">
          申请会费：
          <span style="color:#E94829;margin-left:-0.08rem">￥9980.00</span>
        </div>
        <div class="juese">
          您的任务：招募<span style="color:#E94829">{{ruleInfo3.downMax}}</span>名合伙人
        </div>
      </div>
    </div>
    <div class="speed">
      <div class="speed-wrap">
        ...
        <div class="detail" style="width:79.4vw" v-if="normalactive">
          <div>1、完成招募{{ruleInfo2.vipMax}}名VIP，可获得奖励金298.5元；</div>
          <div>2、可获得所推广VIP及其所分享普通用户消费额的20%分佣。</div>
        </div>
        <div class="detail" style="width:79.4vw" v-if="oneactive">
          <div>1、合伙制分红制度：N+1全额vip奖励金；交易永久性分润机制；管道收入+金融属性；</div>
          <div>2、享受998高级合伙人专属分润体系。</div>
        </div>
        <div class="detail" style="width:79.4vw" v-if="twoactive">
    ...
    <div style="width:100%;padding-top:0.7rem">
  ...
  methods: {
    ...
    checktype: function(type) {
      ...
      if (type == 0) {
        ...
        this.momey = this.ruleInfo2.salePrice;
      } else if (type == 2) {
        this.twoactive = true;
        this.userinfor.type = 3;
        this.userinfor.level = 3;
        this.momey = this.ruleInfo3.salePrice;
      } else {
      }
    },
    /**
    ...
    upgrade() {
      ...
      let id = userinfo.id;
      this.$http
        .post(serviceApi.upgrade, {
          id: id,
          name: _this.userinfor.name, //真实姓名
          idNo: _this.userinfor.idNo, //身份证
          cardNo: _this.userinfor.cardNo, //银行卡号
          applyLevel: String(_this.userinfor.level),
          mobile: _this.userinfor.mobile //手机号
        })
        .then(res => {
          // if(res&&res.)
          console.log(res);
    ...
    getRuleList(level) {
      ...
      this.$http.post(serviceApi.getList).then(res => {
        if (res && res.data && res.data.data) {
          _this.ruleInfo2 = res.data.data[1];
          _this.ruleInfo3 = res.data.data[2];
        }
      });
    }
  }
